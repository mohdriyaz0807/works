class Cricket {
  ...
  let hit1 = () => {
    let players1=['Shane Watson','Faf du Plessis','Ambati Rayudu','Sam Curran','MS Dhoni','Dwayne Bravo','Ravindra Jadeja','N Jagadeesan','Karn Sharma','Shardul Thakur']
    let final = 0;
    let max: any = [];
    let highScore = [];
    for (let j = 0; j < 10; j++) {
    ...
    let maxScore = max;
    let index = highScore.indexOf(max) ;
    (<HTMLInputElement>(
      document.getElementById("player")
    )).innerHTML = players1[`${index}`];
    (<HTMLInputElement>(
  ...
  let hit2 = () => {
    let players2=['Rohit Sharma',' Hardik Pandya',' Jasprit Bumrah',' Krunal Pandya',' Ishan Kishan',' Surya Kumar Yadav',' Rahul Chahar',' Anmolpreet Singh',' Jayant Yadav',' Quinton de Kock',' Kieron Pollard']
    let final = 0;
    let max: any = [];
    let highScore = [];
    for (let j = 0; j < 10; j++) {
